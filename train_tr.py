...
def train(config_file):
    ...
    with tf.device(gpu):
        ...
        GAN.compile(loss=[losses.cycle_loss(), losses.l13D()], loss_weights=[0.5, 0.5],
                    optimizer=Adam(0.0001, beta_1=.5, beta_2=0.9))
    ...
    for pairs in list(total_pairs):
        ...
        src_im, tgt_im, locations, pair_similarity = train_dataset_reader.create_pairs(pairs[0], pairs[1],
                                                                                       config.patch_size)
        ...
        if sim_file.close:
            ...
            sim_file.writelines(
                "Pair:{0} and {1} is CC:{2} \n".format(train_patch_pairs[pairs[0]], train_patch_pairs[pairs[1]],
                                                       pair_similarity))
            sim_file.close()
